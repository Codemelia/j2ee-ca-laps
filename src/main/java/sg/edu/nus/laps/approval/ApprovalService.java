...
public class ApprovalService {
    ...
    // Restrict to current year claims
    @Transactional(readOnly = true)
    public List<OvertimeClaim> getTeamOvertimeClaims(Long managerId, OvertimeClaimStatus... statuses) {
        if (statuses == null || statuses.length == 0) {
            return List.of();
        }
    ...
    // Process leave report export in CSV format
    @Transactional(readOnly = true)
    public byte[] processLeavesExportRequest(Long managerId, List<Long> leaveAppIdList) {
        ...
        if (leaveApps == null || leaveApps.isEmpty()) {
            throw new IllegalArgumentException("Report generation failed on null or empty leaves list");
        }
    ...
    @Transactional(readOnly = true)
    public byte[] processClaimsExportRequest(Long managerId, List<Long> claimIdList) {

        // Retrieve leave apps by Leave ID and Manager ID
        List<OvertimeClaim> teamClaims = otRepo.findByIdInAndEmployeeManagerId(claimIdList, managerId);
        if (teamClaims == null || teamClaims.isEmpty()) {
            throw new IllegalArgumentException("Report generation failed on null or empty claims list");
        }
    ...
    private void appendClaimRow(StringBuilder csvSB, OvertimeClaim claim) {
        csvSB.append(claim.getId())
                .append(',')
            .append(claim.getEmployee().getId())
                .append(',')
            .append(csvEscape(claim.getEmployee().getFirstName() + " " + claim.getEmployee().getLastName()))
                .append(',')
            .append(csvEscape(claim.getWorkedDate() != null ? claim.getWorkedDate().format(DATE_FORMAT) : ""))
                .append(',')
            .append(claim.getClaimedDays())
                .append(',')
    ...
    private void appendLeaveRow(StringBuilder csvSB, LeaveApplication leaveApp, double daysDuration) {
        csvSB.append(leaveApp.getId())
                .append(',')
            .append(leaveApp.getEmployee().getId())
                .append(',')
            .append(csvEscape(leaveApp.getEmployee().getFirstName() + " " + leaveApp.getEmployee().getLastName()))
                .append(',')
            .append(csvEscape(leaveApp.getLeaveType() != null ? leaveApp.getLeaveType().getLeaveType() : ""))
                .append(',')
            .append(csvEscape(leaveApp.getFromDate() != null ? leaveApp.getFromDate().format(DATE_FORMAT) : ""))
                .append(',')
            .append(csvEscape(leaveApp.getToDate() != null ? leaveApp.getToDate().format(DATE_FORMAT) : ""))
                .append(',')
            .append(daysDuration)
                .append(',')
